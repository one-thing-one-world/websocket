<template>
<div class="box">
  <div class="container"v-if="userlists.length>0">
    <div class="left" v-if="userlist.length>0">
      ...
      <div class="username" v-for="(item,index) in userLists" :key="index">
        <div class="avatar">
...
export default {
  ...
  mounted() {
    this.userlist = JSON.parse(this.$route.params.user);
    this.userSelf = this.userlist[
      JSON.parse(this.$route.params.user).length - 1
    ];
    console.log(JSON.parse(this.$route.params.user));
  },
  sockets: {
    loginSuccess(data) {
      if (data.msg !== "登陆失败") {
